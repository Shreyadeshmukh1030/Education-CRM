...
base_dir = "C:/Users/HP/Desktop/Education-CRM/templates/students"
os.makedirs(base_dir, exist_ok=True)

models = {
    'fee': {'title': 'Fee Invoices', 'fields': ['student', 'title', 'amount', 'due_date', 'is_paid'], 'obj': 'invoice'},
}
...
<div class="dashboard-header animate-fade-in stagger-1" style="margin-bottom: 24px;">
    <a href="{{% url 'fees' %}}" style="color: var(--text-muted); text-decoration: none; display: inline-flex; align-items: center; margin-bottom: 10px;">
...
<div class="card animate-fade-in stagger-2" style="max-width: 600px;">
    <form method="post" enctype="multipart/form-data">
        ...
        <div style="margin-top: 24px; display: flex; gap: 10px;">
            ...
            <a href="{{% url 'fees' %}}" class="btn" style="background-color: var(--background-color); color: var(--text-color); border: 1px solid var(--border-color);">Cancel</a>
        </div>
    </form>
</div>

<style>
    form input[type="text"], form input[type="number"], form input[type="email"], form input[type="date"], form input[type="time"], form select, form textarea, form input[type="checkbox"] {{
        width: 100%;
        padding: 10px 15px;
...
print("Fee templates generated successfully!")
