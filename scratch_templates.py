...
base_dir = "C:/Users/HP/Desktop/Education-CRM/templates/academics"

models = {
    'section': {'title': 'Sections', 'fields': ['name'], 'obj': 'section_obj'},
    'subject': {'title': 'Subjects', 'fields': ['name', 'code'], 'obj': 'subject_obj'},
    'syllabus': {'title': 'Syllabus Topics', 'fields': ['name', 'subject', 'class_assigned', 'status'], 'obj': 'topic_obj'},
}
...
# Add missing confirm_delete for Class, Assignment, Material
for model in ['class', 'assignment', 'material']:
    with open(os.path.join(base_dir, f"{model}_confirm_delete.html"), "w") as f:
        f.write(delete_template.format(model=model, model_cap=model.capitalize()).replace("{{%", "{%").replace("%}}", "%}"))
